...
set1 = np.vstack((xvector,np.array(errorvectors[0,:100])))
#input(set1)
np.savetxt('Aug1Data/IS Optimization Errors/run_-250_to_50.txt',np.transpose(set1))
...
for dir in dirs:
    ...
    for test in tests2:
        ...
        k+=1
    plt.title(dir+' beta vs IS error')
    plt.plot(np.linspace(-50,50,100),errorvectors[0,100:],'k.')
    plt.plot(xvector,errorvectors[0,:100],'k.')
    plt.show()
    
set2 = np.vstack((np.linspace(-50,50,100),np.array(errorvectors[0,100:])))
print(set2)

np.savetxt('Aug1Data/IS Optimization Errors/run_-50_to_50.txt',np.transpose(set2))