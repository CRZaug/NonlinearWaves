...
errorvectors = np.zeros((1,len(files)+100))
...
set1 = np.vstack((xvector,np.array(errorvectors[0,:len(files)])))

np.savetxt('Aug2Data/IS Optimization Errors/run_-300_to_300.txt',np.transpose(set1))
...
for dir in dirs:
    ...
    for test in tests2:
        ...
        k+=1
    plt.title(dir+' beta vs IS error')
    plt.plot(np.linspace(-50,50,100),errorvectors[0,157:],'k.')
    plt.plot(xvector,errorvectors[0,:157],'k.')
    plt.show()
    
set2 = np.vstack((np.linspace(-50,50,100),np.array(errorvectors[0,len(files):])))
print(set2)
np.savetxt('Aug2Data/IS Optimization Errors/run_-50_to_50.txt',np.transpose(set2))