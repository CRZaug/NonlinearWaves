...
for sd in subdirs:
    ...
    for f in files:
        ...
        interpolated = np.transpose(np.loadtxt(sd+'/Interpolated/'+f[-10:]).view(float))
        interpx = interpolated[0]*0.001 # Frequencies in Hz
        interpy = interpolated[1]*(0.01)**2 # Variance in m^2
        
        interpy1 = 1/2*np.append(np.flip(interpy,0),interpy)
        interpx1 = np.append(-1*np.flip(interpx,0),interpx)
        f,a=plt.subplots()
        a.plot(interpx1,interpy1)
        plt.show()
...
#         L = 3600*3 # The period in seconds
