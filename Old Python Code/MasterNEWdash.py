# THIS IS AN OLD VERSION THAT I NO LONGER USE
    ...
    for whichset in ['Aug1Data/'+masterdir,'Aug2Data/'+masterdir,'JulyData/'+masterdir]:
        ...
        for key in key2:
            ...
            for i in range(len(plotter1)):
                ...
                x = VALUES[0]
                y = VALUES[1]
                ax1[i].plot(x,y,markersize = sizes[dispind])
                ax2[po].set_dashes(disp[dispind])
                ax1[i].set_title(titles1[i])
        ...
        for key in key2: 
            ...
            for po in range(len(titles2)):
                ax2[po].plot(x,sideband7[po],markersize = sizes[dispind])
                ax2[po].set_dashes(disp[dispind])
...
def sberror(masterdir):
    ...
    for whichset in ['Aug1Data/'+masterdir,'Aug2Data/'+masterdir,'JulyData/'+masterdir]:
            ...
            dir = ['dGT dimCQ/', 'dNLS dimCQ/', 'Dysthe dimCQ/','NLS dimCQ/','vDysthe dimCQ/']
            
            # Set up some values for file writing
            titles = ['dGT', 'dNLS', 'Dysthe', 'NLS', 'vDysthe']
            errorvect = np.array([])
