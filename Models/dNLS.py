...
def linear(initial_condition,deltat,expconsts,Del): 
    uhat0=fft(initial_condition)
    return ifft(uhat0*np.exp(deltat*(1j*expconsts**2-Del)))
...
def first(initial_condition,deltat,steps,expconsts,Del):
    
    unew=initial_condition
    for j in range(0,steps):
        unew=linear(unew,deltat,expconsts,Del)
        unew=nonlinear(unew,deltat)
    return unew


#______________SECOND ORDER OPERATOR SPLITTING__________________
def second(initial_condition,deltat,steps,expconsts,Del):
    # Preparation before the operator splitting
    sdt = deltat*0.5 # Creates a new timestep size
    # The first part
    unew = linear(initial_condition,sdt,expconsts,Del) # linear part
    ...
    for i in range(0,steps-1): # The second part; the bulk of the loop (includes every timestep except first and last)
        unew = linear(unew, deltat,expconsts,Del) # Uses unew from previous loop 
        unew = nonlinear(unew,deltat) # Outputs a new unew
    ufinal = linear(unew,sdt,expconsts,Del) # This is the final result

    return ufinal


#________________SIXTH ORDER OPERATOR SPLITTING___________________
def sixth(initial_condition,deltat,steps,expconsts,Del): # Take in the initial space profile and timevector 
    ...
    unew= linear(initial_condition,lt*w3,expconsts,Del) # linear part, the first step
    ...
    for i in range(0,steps-1): # The second step, loop through everything except the first and last timestep
        ...
        unew = linear(unew, lt*(w3+w2),expconsts,Del)
        unew = nonlinear(unew, nlt*w2)
        unew = linear(unew, lt*(w1+w2),expconsts,Del)
        unew = nonlinear(unew,nlt*w1)
        unew = linear(unew, lt*(w1+w0),expconsts,Del)
        unew = nonlinear(unew, nlt*w0)
        unew = linear(unew, lt*(w0+w1),expconsts,Del)
        unew = nonlinear(unew, nlt*w1)
        unew = linear(unew, lt*(w1+w2),expconsts,Del)
        unew = nonlinear(unew, nlt*w2)
        unew = linear(unew, lt*(w2+w3),expconsts,Del)
        unew = nonlinear(unew, nlt*w3)
        unew = linear(unew, nlt*w3,expconsts,Del) # Note different timestep
    ...
    unew = linear(unew, lt*(w2+w3),expconsts,Del)
    unew = nonlinear(unew, nlt*w3)
    ufinal = linear(unew, lt*w3,expconsts,Del) # This is the final result
    return ufinal
