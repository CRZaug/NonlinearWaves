...
def first(u0,deltat,steps,rk4steps,k,expconsts,epsilon,per):
    
    linexpconsts=1j*expconsts**2
    unew = u0
...
def second(u0,deltat,steps,rk4steps,k,expconsts,epsilon,per):
...
def sixth(u0,deltat,steps,rk4steps,k,expconsts,epsilon,per):
...
def Dysthesoln(xspace,b0,l,rho,epsilon,t):
    
    return b0*np.exp(1j*l*xspace+1j*rho+1j*(-l**2+4*b0**2*(1+6*l*epsilon))*t)

#________________________INITIAL DATA__________________________
def Dysthe_id(gridnum,L,b0,l,rho,epsilon):
    
    xspace=np.linspace(-L/2.0, L/2.0-L/gridnum, gridnum,dtype = complex)
    initialprofile = Dysthesoln(xspace,b0,l,rho,epsilon,0)
...
def runDysthe(PARAMS,simtimes,u0,k,expconsts,epsilon,per):
    ...
    for t in range(1,num_o_times):
        ...
        sim_Dysthe = sixth(u0,deltat,steps,rk4steps,k,expconsts,epsilon,per)
