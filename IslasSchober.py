...
def rk4(u0, dt, rk4steps,k,expconsts,bet,epsilon):
    # deltat is the final timestep, esentially.
    t = 0
    for i in range(rk4steps):
        km1 = drhs(u0, t,k,expconsts,bet,epsilon)
        km2 = drhs(u0+(dt/2)*km1, t+dt/2,k,expconsts,bet,epsilon)
        km3 = drhs(u0+(dt/2)*km2, t+dt/2,k,expconsts,bet,epsilon)
        km4 = drhs(u0+(dt)*km3, t+dt,k,expconsts,bet,epsilon)
...
def drhs(u0, t,k,expconsts,bet,epsilon):
    ...
    return 4j*modu2*u0+epsilon*(32*modu2*u0p+(-8j-bet)*u0*der_hil(modu2,expconsts,k,1))
    

#____________________________NONLINEAR_____________________________
def nonlinear(u0,deltat,rk4steps,k,expconsts,bet,epsilon):
    # This equation has a free paramenter beta, called bet
    dt = deltat/rk4steps
    return rk4(u0, dt, rk4steps,k,expconsts,bet,epsilon)
...
def first(u0,deltat,steps,rk4steps,k,expconsts,bet,epsilon,Del,per):
    # This equation has a free paramenter beta, called bet
    
    linexpconsts=(1j*expconsts**2-Del)
    unew = u0
    
    for j in range(steps):
        unew=linear(unew,deltat,linexpconsts,per)
        unew=nonlinear(unew,deltat,rk4steps,k,expconsts,bet,epsilon)
        
    return unew


#__________________SECOND ORDER OPERATOR SPLITTING____________________
def second(u0,deltat,steps,rk4steps,k,expconsts,bet,epsilon,Del,per):
    # This equation has a free paramenter beta, called bet
    ...
    unew = nonlinear(unew,deltat,rk4steps,k,expconsts,bet,epsilon) # nonlinear part, which outputs unew to be used in the next loop
    ...
    for i in range(0,steps-1): # The second part; the bulk of the loop (includes every timestep except first and last)
        ...
        unew = nonlinear(unew,deltat,rk4steps,k,expconsts,bet,epsilon) # Outputs a new unew
...
def sixth(u0,deltat,steps,rk4steps,k,expconsts,bet,epsilon,Del,per):
    # This equation has a free paramenter beta, called bet
    ...
    for i in range(0,steps-1): # The second step, loop through everything except the first and last timestep
        unew = nonlinear(unew, nlt*w3,rk4steps,k,expconsts,bet,epsilon)
        unew = linear(unew, lt*(w3+w2),linexpconsts,per)
        unew = nonlinear(unew, nlt*w2,rk4steps,k,expconsts,bet,epsilon)
        unew = linear(unew, lt*(w1+w2),linexpconsts,per)
        unew = nonlinear(unew,nlt*w1,rk4steps,k,expconsts,bet,epsilon)
        unew = linear(unew, lt*(w1+w0),linexpconsts,per)
        unew = nonlinear(unew, nlt*w0,rk4steps,k,expconsts,bet,epsilon)
...
def runIS(PARAMS,simtimes,u0,k,expconsts,bet,epsilon,Del,per):
    ...
    # Question: This seems slow. Should I re-write my code to print out every deltat?
    for t in range(1,num_o_times):
        ...
        sim_IS = sixth(u0,deltat,steps,rk4steps,k,expconsts,bet,epsilon,Del,per)
        r_IS[t,:] = sim_IS
